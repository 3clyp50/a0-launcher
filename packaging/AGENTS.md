...
- Use `A0_LAUNCHER_APP_VERSION` and `A0_LAUNCHER_RELEASE_TAG` for CI-provided
  release versions. Two-segment tags such as `v1.2` are the public release
  shape; build them as semver `1.2.0` where tooling requires it, but stage
  public assets with release version `1.2`.
